...
from conv_lstm_model1 import model
...
class Tracker:
    def __init__(self, path, weapons):
        ...
        self.path = path
        self.model_link = "./yolo/best_v02.pt"
    ...
    def load_model(self):
        ...
        model = YOLO(self.model_link, verbose=False)
        model.fuse()
    ...
    def get_result(self, results, frame):
        ...
        for result in results.boxes.data.tolist():
            ...
            if score > self.threshold:
                res_array.append(([int(x1), int(y1), int(x2-x1), int(y2-y1)], float(score), int(class_id)))
    ...
    def draw_target(self, x_center, y_center, frame, colour):
        ...
        cv2.circle(frame, center, 9, colour, 2)
        cv2.circle(frame, center, 7, colour, 2)
        cv2.circle(frame, center, 5, colour, 2)
        cv2.circle(frame, center, 3, colour, 2)
        cv2.circle(frame, center, 1, colour, -1)

    def draw(self, frame, resutls, priority):
        if resutls is not None:

            for (bboxes, idx, class_id) in resutls:
                x1, y1, x2, y2 = map(int, bboxes)
                ...
                text = f"{idx}:{self.yolo_names[int(class_id)]}:{action}"
    ...
    def update_dict(self, resutls):
        current_idx = {idx for (_, idx, _) in resutls}
    ...
    def predict_command(self, array) -> str:
    ...
    #
    # def collet_centers(self):
    #     centers = []
    #     for _, value in self.coordinates.items():
    #         x1, y1, x2, y2 = value
    #         x_center, y_center = self.get_center(x1, y1, x2, y2)
    #         centers.append((x_center, y_center))
    #
    #     return centers
    #
    # def navigate_shells(self, centers):
    #     if len(centers) > 0:
    #         for x, y in centers:
    ...
    def info_window(self, amount, amount_of_actions, tactical_maneuver, command, priority):
        ...
        cv2.putText(window, amount, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
    ...
    def _run(self):
        ...
        assert cap.isOpened(), f"cannot open {self.path}"

        fps = cap.get(cv2.CAP_PROP_FPS)
        self.frame_dt = 1.0 / fps if fps and fps > 1e-3 else 1 / 30.0
        self._ended = False
        ...
        try:
            while True:
                ...
                for (bboxes, idx, class_id) in resutls_array:
                    self.vehicles[idx] = self.yolo_names[int(class_id)]

                    x1, y1, x2, y2 = self.resize_frame(bboxes, h, w)
                    ...
                    if crop.size:
                        ...
                        if len(self.frames[idx]) == self.frames_length:
                            frames_clip = list(self.frames[idx])
                            threading.Thread(target=self.prediction, args=(frames_clip, idx), daemon=True).start()
                            self.frames[idx].clear()

                tank, ifv, apc = self.count_vehicles()
                features = self.encode_items(tank, ifv, apc)
                command = self.predict_command(features)
                ...
                vis = frame.copy()
                vis = self.draw(vis, resutls_array, priority)
                self.draw_total_coordinates(vis, resutls_array, h, w)
                _ = self.info_window(amount, amount_of_actions, tactic_prediction, command, priority)

                self.last = vis

                frame_idx += 1
                target = t0 + frame_idx * self.frame_dt
